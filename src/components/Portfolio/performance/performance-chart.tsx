...
export default function PerformanceDashboard() {
    ...
    const performanceData: PerformanceData[] = [
        { date: "Apr 2025", murakkabAverage: -3.02, murakkabAverageSecondary: -3.02, sp500: 3.02, myPortfolio: -3.02 },
        { date: "Apr 2025", murakkabAverage: -4.15, murakkabAverageSecondary: -2.89, sp500: 3.02, myPortfolio: -3.02 },
        { date: "Apr 2025", murakkabAverage: -2.75, murakkabAverageSecondary: -3.02, sp500: 3.02, myPortfolio: -3.02 },
        { date: "Apr 2025", murakkabAverage: -3.45, murakkabAverageSecondary: -3.02, sp500: 3.02, myPortfolio: -3.02 },
        { date: "Apr 2025", murakkabAverage: -3.89, murakkabAverageSecondary: 3.02, sp500: -3.02, myPortfolio: 3.02 },
        { date: "Apr 2025", murakkabAverage: 3.02, murakkabAverageSecondary: 3.02, sp500: 3.02, myPortfolio: 3.02 },
    ]
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="mb-8 col-span-4 overflow-x-scroll lg:overflow-auto">
                        ...
                        {performanceData.map((data, index) => (
                            <div key={index} className="flex flex-col items-center">
                                <div className="h-36 w-full bg-gray-100 relative flex flex-col justify-center items-center">
                                    ...
                                    {showMurakkab && data.murakkabAverage > 0 && (
                                        <div
                                            ...
                                            style={{ height: `${getBarHeight(data.murakkabAverage)}%` }}
                                        ></div>
                                    )}
                                ...
                                <div className="flex flex-col gap-1 mt-2">
                                    {showMurakkab && (
                                        <div
                                            className={`flex items-center gap-1 ${data.murakkabAverage > 0 ? "text-green-500" : "text-red-500"}`}
                                        >
                                            {data.murakkabAverage > 0 ? "▲" : "▼"} {Math.abs(data.murakkabAverage).toFixed(2)}%
                                        </div>
                                    )}

                                    {showMurakkab && (
                                        <div
                                            className={`flex items-center gap-1 ${data.murakkabAverageSecondary && data.murakkabAverageSecondary > 0 ? "text-green-500" : "text-red-500"}`}
                                        >
                                            {data.murakkabAverageSecondary && data.murakkabAverageSecondary > 0 ? "▲" : "▼"}{" "}
                                            {data.murakkabAverageSecondary ? Math.abs(data.murakkabAverageSecondary).toFixed(2) : "0.00"}%
                                        </div>
                                    )}

                                    {showSP500 && (
                                        <div className={`flex items-center gap-1 ${data.sp500 > 0 ? "text-green-500" : "text-red-500"}`}>
                                            {data.sp500 > 0 ? "▲" : "▼"} {Math.abs(data.sp500).toFixed(2)}%
                                        </div>
                                    )}

                                    {showPortfolio && (
                                        <div
                                            className={`flex items-center gap-1 ${data.myPortfolio > 0 ? "text-green-500" : "text-red-500"}`}
                                        >
                                            {data.myPortfolio > 0 ? "▲" : "▼"} {Math.abs(data.myPortfolio).toFixed(2)}%
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="flex justify-start">
                        <span className="text-gray-700 font-medium">{minValue.toFixed(2)}%</span>
                    </div>
                </div>
            </div>
        </div>
